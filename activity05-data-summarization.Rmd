...
**Response**: It causes the whole data set to print so we can see it.
...
```{r summary_earnings}
 college_recent_grads %>%
  summarise(median_by_major = median(median), sd_by_major = sd(median), mean_by_major = mean(median), n_of_majors = n(), min_med_earnings = min(median), max_med_earnings = max(median))
```
...
**Response**: I believe that this might be a normal distribution, with center around 36000, maybe some slight skewness
...
**Response**: They all seem to be pretty close in median earnings, engineering is quite a bit higher than the rest, but most of them are within 10000 of the overall median.
...
**Response**: When trying to better express the data, make it "publication ready"
...
```{r major_boxplot}
college_recent_grads %>% 
  # boxplot and jitter of median earnings
...
**Response**: They are in fact all pretty close as far as median goes, some have very different spreads compared to others, like interdisciplinary only containing one major.
...
college_recent_grads %>% 
  group_by(major_category) %>% 
  summarise(sum_total = n(), median_sharewomen = median(sharewomen), median_unemp = median(unemployment_rate), median_median = median(median))
